jQuery(document).ready(function($){
    ...
    $('#social_link_Internal_Link').change(function(){
        if($(this).val() !== '0' && $('#social_link_External_Link').val() !== '') {
            if(confirm('You have an external link set. Clicking ok will remove the external link and use this page.')){
                $('#social_link_External_Link').val('');
            }
            else{
                $(this).val(0);
            }
    ...
    $('form#post').submit(function(e){
        ...
        if($('#social_link_External_Link').val() === '' && $('#social_link_Internal_Link').val() === '0'){
	...
	if( $('.wp-list-table th.column-sort').length > 0 ) {
		...
		$('.wp-list-table tbody').sortable({
			...
			stop: function(event, ui){
				...
				$.ajax({
					data: "&rows=" + JSON.stringify(rows) + "&action=sort_social_link&social_link_nonce=" + social_link_nonce.Nonce,
					url: ajaxurl,
					type:'POST',
					statusCode: {
						200: function() {
