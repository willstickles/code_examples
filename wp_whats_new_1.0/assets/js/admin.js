		...
		$('.wp-list-table tbody').sortable({
			...
			stop: function(event, ui){
                ...
                $(tableRows).each(function (index, row) {
                    var num = $(this).find('td.column-sort').html();
                    if(isNaN(num)) {
                        isValid = false;
                    }
                ...
                if(!isValid) {
                    alert('Sorting error. Sort values can only be numeric.');
                    return false;
                }
				...
				$.ajax({
					data: "&rows=" + JSON.stringify(rows) + "&action=sort_whats_new&whats_new_nonce=" + whats_new_nonce.Nonce,
					url: ajaxurl,
					type:'POST',
					statusCode: {
						200: function() {
